...
###		FOR A DSM THAT SAMPLES THE CONTOUR AT PLANES OR LOCATIONS DIFFERENT FROM THE AXIAL SLICES IN THE CONTOUR DICOM
#start by making a coarsely anchored spline along the COM path
	#this smoothes out the path
spl1 = pv.Spline(COM,int(len(COM)/5))	
#next, we make a densely anchored spline based on the path of the smoothed centerline
	#this makes it so we have a smooth path with small step sizes we can use to define equally spaced sampling points
spline = pv.Spline(spl1.points,1000)
#finally we get a list of points along the centerline spline spaced 3mm from one another
samppoints = DSM_functions.sample_centerline(spline,3,steptype='dist')

#Next we generate a dictionary of points along the centerline to center sampling planes and the normals of these planes
Slices = DSM_functions.sample_contour_surface(volume,spline.points,samppoints,NonPlanar=True)
#We follow this with a check and correction for any overlapping slices to avoid sampling the same point on 2 different planes
	#such overlaps occur most frequenctly near the ends of the contour or at tight bends
Slices = DSM_functions.correct_slice_overlaps(Slices,samppoints,spline)
...
DSM_functions.plot_single_DSM(DSM,'Non-Planar DSM','DSMpreview')	#DSM to plot, title on plot, save name for the file
